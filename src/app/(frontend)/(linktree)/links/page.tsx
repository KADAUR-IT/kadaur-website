...
export default async function LinkTreePage() {
  ...
  const socialMediaArray = socialMedia.map((sm, index) => {
    return (
      <a key={sm.id} href={sm.socialMediaLink as string}>
        <FontAwesomeIcon icon={sm.socialMediaSelect as IconProp} className="h-[32px]" />
      </a>
    )
  })
  ...
  const linksArray = linktreeData.links!.linksArray!.map((link, index) => {
    return (
      <NavLink
        key={index}
        large={link.isLarge as boolean}
        href={link.link as string}
        thumbnail={link.image as Media}
      >
  ...
  return (
    <div className="flex flex-col gap-2 items-center max-w-xl bg-(--color-blue) min-h-screen w-full rounded p-2 ">
      ...
      <h2 className="text-(--color-white)! text-center">
        Expert en gestion et en pilotage de projets IT et spécialisé en Infrastructure.
      </h2>
      <div className="flex items-center justify-center gap-4 p-4">{socialMediaArray}</div>
      <VisitCard />

      <div className="flex flex-col gap-2 w-full">{linksArray}</div>
    </div>
  )
}
